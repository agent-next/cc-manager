...
describe("AgentRunner", () => {
  ...
  it("createTask generates 16-char ID by default (R3)", () => {
    const task = createTask("test task");
    assert.strictEqual(task.id.length, 16, `expected 16-char id, got ${task.id.length}: ${task.id}`);
    assert.ok(/^[0-9a-f]+$/.test(task.id), "id should be lowercase hex");
  });

  it("createTask preserves custom ID when provided", () => {
    const task = createTask("test", { id: "custom-id" });
